...
/**
 * Экран со списком статей.
 *
 * Представляет собой список статей по теме здоровья и образа жизни, каждая из которых содержит заголовок,
 * краткое содержание и изображение. Этот экран использует компонент [ArticleItem] для отображения
 * каждой статьи в виде отдельного элемента списка.
 *
 * Статьи охватывают различные темы, связанные с качеством сна, влиянием технологий на сон,
 * рекомендациями по температуре в спальне, управлением потреблениями кофеина и алкоголя и пользой физической активности.
 */
@Composable
fun ArticlesScreen() {
    val articles = listOf(
